...
export const useStartupProfile = () => {
    ...
    const fetchStartupProfiles = useCallback(async () => {
        setLoading(true);
        setError(null);

        try {
            const response = await axiosInstance.get('/startup');
    ...
    const fetchLeadersWithStartupProfile = useCallback(async (page: number = 1) => {
        setLoading(true);
        setError(null);

        try{
            const response = await axiosInstance.get(`leaders?page=${page}`);
            return response.data;
        } catch (err) {
            const errormessage = (err as any).response?.data?.message || 'An Unknown error occured';
            setError(errormessage);
